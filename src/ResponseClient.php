...
final class ResponseClient
{
     ...
     */
    public function __construct($code, $duration, array $headers = array(), $body = null, $error = null, $request = null) {
        ...
        if ($error !== null)
            return;
        if ($code >= 400) {
            $this->error =$body === null? self::$statusTexts[$code]:$body;
            return ;
        }
        if (self::isJson($headers)) {
            try {
                $jsonData = is_array($body) ? $body : self::bodyJson($body);
                if ($code >= 400) {
                    $this->error = $body;
     ...
     * @return mixed
     */
    public static function bodyJson($body) {
